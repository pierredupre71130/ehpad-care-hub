...
import { MessageSquare, Home, Send, ArrowLeft, Loader2, Trash2, CheckSquare, Square, X as XIcon, Plus, Search } from 'lucide-react';
...
function AdminMessagingPageInner() {
  ...
  // Liste des utilisateurs (pour la composition d'un nouveau message)
  const { data: usersData } = useQuery<{ users: { id: string; email: string; role: string | null; display_name: string }[] }>({
    queryKey: ['admin-users-for-messaging'],
    queryFn: () => fetch('/api/admin/users').then(r => r.json()),
    enabled: profile?.role === 'admin' && showCompose,
    staleTime: 60_000,
  });
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[85vh] flex flex-col"
            ...
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                <h2 className="font-semibold text-slate-900">Nouveau message</h2>
                <p className="text-xs text-slate-500">Choisir un utilisateur</p>
              </div>
            ...
            <div className="overflow-y-auto p-3 space-y-1.5">
              {!usersData ? (
                ...
                const term = composeSearch.toLowerCase().trim();
                ...
                if (filtered.length === 0) {
                  return <p className="text-sm text-slate-400 text-center py-6">Aucun utilisateur trouvé</p>;
                }
                return filtered.map(u => (
                  <button key={u.id}
                    ...
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 bg-slate-100 text-slate-600">
